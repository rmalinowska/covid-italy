...
def make_correlation_plot(df, output_filename):
	...
	fig = go.Figure()
	fig.add_trace(go.Scatter(x = final_df["density"], y = final_df["total_cases_per_10k"],
			  mode = 'markers',
			  name = 'total cases per 10k people',
			  marker = dict( color = plotly_color_palette),
			  text = final_df["region_name"]))
	fig.add_trace(go.Scatter(x = final_df["density"], y = regression(final_df["density"], final_df["total_cases_per_10k"]),
			  mode = 'lines',
			  name = 'linear regression line'))
			  	
	fig.update_layout(
    title="Correlation between density of the region and total number of cases per 10 000 people in regions of Italy",
    xaxis_title="number of people / 1km2",
    yaxis_title="total number of cases per 10 000 people")
...
if __name__ == "__main__":
	make_correlation_plot(final_df, "../for_report/densities_vs_cases.html")
