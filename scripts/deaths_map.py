...
for region in region_names:
    for month_year in months_years:
        ...
        grouped_total_deaths.append(groups[(month_year, region)]["total_deaths_per_10k"].tolist()[-1])
...
def plot_map(df):
    fig = px.choropleth_mapbox(df, geojson=counties, locations='region_name', featureidkey="properties.Nome", color='total_deaths_per_10k',
                            color_continuous_scale="Viridis",
                            range_color=(0, max(covid["total_deaths_per_10k"])),
                            mapbox_style="carto-positron",
                            zoom=5.4, center = {"lat": 42.0902, "lon": 9.1129},
                            opacity=0.5,
                            labels = {'total_deaths_per_10k':'total deaths per 10 000{}'.format(get_sub('2'))},
                            animation_frame = "month_year"
                            )
    fig.update_layout(margin={"r":0,"t":0,"l":0,"b":0})
    fig.write_html("deaths_map.html")
    fig.write_image("deaths_map.png")
